  ...
  return (
    <section className="relative min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-kino-panel shadow-card">
      <div className="image-text-surface relative min-h-[430px] w-full overflow-hidden sm:min-h-[400px] md:h-[480px] md:min-h-0">
        ...
        <div className="relative z-10 flex min-h-[430px] min-w-0 flex-col justify-end gap-4 p-4 sm:min-h-[400px] sm:p-6 md:h-full md:min-h-0 md:flex-row md:items-end md:gap-6 md:p-10">
          {poster && (
          ...
          <div className="min-w-0 max-w-2xl space-y-3 card-animate sm:space-y-4">
            ...
            <h1 className="break-words text-display text-3xl font-bold leading-tight text-white sm:text-4xl md:text-6xl">
              {title}
            </h1>
            ...
            {item.overview && (
              <p className="line-clamp-2 max-w-xl text-sm text-white/80 sm:line-clamp-3 sm:text-base md:text-lg">
                {item.overview}
              </p>
            )}
            <div className="flex min-w-0 flex-wrap gap-2 pt-1 sm:gap-3 sm:pt-2">
              <Link href={`/title/${mediaType}/${item.id}`} className="btn-primary !px-4 !py-2 text-sm sm:!px-5">
                <PlayIcon /> {t("hero.details")}
              </Link>
              <Link href="/ce-soir" className="btn-ghost !px-4 !py-2 text-sm sm:!px-5">
